$(async () => {
  ...
  const audios = files.filter((v, i, arr) => v && !v.toLowerCase().startsWith('bgm') && (v.endsWith('.mp3') || v.endsWith('.wav')) && arr.indexOf(v) === i);
  const bgm = files.find(i => i.toLowerCase().startsWith('bgm'));
