...
class GetSearchRequest extends Api {
  ...
  async exec(req: Request, res: Response): Promise<void> {
    let name: string = decodeURI(req.query.name);
    ...
    try {
      ...
      if (itemClass && typeof itemClass !== 'function')
        throw {
          code: 403,
          message: 'Invalid item class from [soul,eidolon,kamihime,weapon]. Perhaps you are confusing this with `type` parameter?'
        };
      ...
      if (type)
        results = results.filter(el => String(el.element).toLowerCase() === type.toLowerCase());

      if (rarity)
        results = results.filter(el =>  String(el.element).toLowerCase() === rarity.toLowerCase());
